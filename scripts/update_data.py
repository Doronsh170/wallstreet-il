...
1. ONE bullet of context only: last session closing levels (S&P 500, Nasdaq, Dow with %). This is the ONLY backward-looking bullet allowed.
1. Then ALL remaining bullets must be forward-looking:
...
1. Do NOT include multiple bullets about what happened yesterday. One context bullet is enough.
...
1. Then the key events coming THIS week: Fed decisions, economic data (NFP, CPI, PMI, GDP), earnings reports, trade/tariff deadlines, geopolitical developments.
1. For each event: include the specific day and Israel time when known (e.g. “יום רביעי 21:00 שעון ישראל”).
1. End with notable companies expected to report earnings this week.
...
1. Then macro data published this week: employment, inflation, PMI, GDP, consumer sentiment — actual numbers and market reaction.
1. Then key events that moved markets: geopolitics, Fed comments, trade/tariff news, commodity moves (oil, gold with numbers).
1. Then notable earnings reports: company ($TICKER), stock move %, key result.
1. End with sector rotation or institutional activity if relevant.
- Do NOT repeat information across bullets. Each bullet = one unique fact.
